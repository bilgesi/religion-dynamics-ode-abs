#!/usr/bin/env python3
"""
Fit Finland historical data using reduced ODE model (baseline + piecewise).
Similar to Sweden/JW historic fit scripts.
"""
...
DATA = Path("data/raw/finland/finland_4groups_shares_long.csv")
...
def fit_piecewise(years, y_obs, break_year_candidates):
    ...
    for by in break_year_candidates:
        ...
        if by <= years[0] + 2 or by >= years[-1] - 2:
            continue
...
def main(group="none"):
    ...
    df = pd.read_csv(DATA)
    df = df[df["group"] == group].sort_values("year")
    
    if len(df) == 0:
        print(f"ERROR: No data found for group '{group}'", file=sys.stderr)
    ...
    print(f"Fitting Finland data for group '{group}' ({len(years)} points, {years.min()}-{years.max()})")
    ...
    # piecewise (grid-search)
    print("Fitting piecewise model (grid search for break_year)...")
    by_candidates = range(int(years[0]) + 3, int(years[-1]) - 2)
    ...
    pred_path = OUT / f"finland_{group}_predictions.csv"
    ensure_dirs(pred_path.parent)
    pred.to_csv(pred_path, index=False)

    # save summary
    summary = {
        "dataset": "Finland: Population 31 Dec by religious community (1990-2019)",
        "group": group,
    ...
    sum_path = OUT / f"finland_{group}_fit_summary.json"
...
if __name__ == "__main__":
    ...
    ap.add_argument("--group", default="none",
                    choices=["none", "christianity", "islam", "other"])
    args = ap.parse_args()
