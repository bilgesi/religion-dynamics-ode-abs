"""
Run Scenarios B and C together with ODE and ABS.

Generates time series data and figures for both scenarios in a single run.
...
def main():
    print("=" * 60, flush=True)
    print("Running scenarios B and C...", flush=True)
    print("=" * 60, flush=True)
    
    cfg = DEFAULT_CONFIG
    ensure_dirs(cfg.outputs_dir, cfg.runs_dir, cfg.figs_dir)

    for key in ["B"]:  # Only Scenario B
    ...
    print("\n" + "=" * 60, flush=True)
    print("Scenarios B and C completed!", flush=True)
    print("=" * 60, flush=True)
